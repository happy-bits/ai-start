...
test('edit a contact', async ({ page }) => {
    ...
    await page.getByRole('article', { name: 'Contact: Erik Andersson' }).getByLabel('Expand contact details').click();
    ...
    await page.getByRole('textbox', { name: 'Email for Erik Anderssonnnn' }).fill('erik.andersson@volvoab.see');
